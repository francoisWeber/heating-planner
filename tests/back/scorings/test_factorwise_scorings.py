...
def test_factorwise_scoring_by_type(df, factors):
    scores = FactorsScoringStrategy._by_trend(df, factors)
...
def test_factorwise_scoring_by_factor_optimal_range_discrepancy(df, factors, optimal_ranges):
    scores = FactorsScoringStrategy._by_optimal_range_discrepancy(df, factors, optimal_ranges)
    expected = pd.DataFrame(
        {
            "f1": [0.1, 0, 0, 1],
            "f2": [0, 0, 0.5, 1.5],
...
def test_factorwise_scoring_by_factor_reference_values(df, factors):
    ...
    scores = FactorsScoringStrategy._by_historical_value(df_ref, df_proj, factors)
    expected = pd.DataFrame(
        {
