...
from heating_planner.back.scoring.processor import (
    Contrast,
    apply_binary_masks,
    process_score,
)
from heating_planner.back.scoring.scaler import ScoreScalingStrategy
from heating_planner.crs import GPS_CRS, METRIC_CRS
...
class OptionsDisplayer:
    ...
    def display():
        ...
        with scores_col:
            ...
            factors_scoring_strategy = (
                OptionsDisplayer.display_factors_scoring_strategy()
            )
        with scaling_col:
    ...
    def display(factors_continous: List[Factor], factors_binary: List[Factor]):
        ...
        with st.container(border=True):
            for i, factor in enumerate(factors_binary):
                factors_binary_activation[factor] = st.toggle(
                    "With: " + factor.description, value=True
                )
            ...
            with st.form("weight-form"):
                ...
                for i, factor in enumerate(factors_continous):
                    ...
                    with factors_cols[i % FACTOR_WEIGHTS_NCOLS]:
                        factors_continuous_coefs[factor] = st.slider(
                            factor.description[:50],
                            0,
                            3,
                            step=1,
                            value=factors_continuous_coefs[factor],
                        )
...
class MapDisplayer:
    ...
    def display(
        ...
        ds_hist_binary,
        ds_proj_continuous,
        factor_scoring_strategy,
        factors_scaling,
        fusion_strategy,
        contrast,
        factors_continuous_coefs,
        factors_binary_activation,
    ) -> Tuple[gpd.GeoDataFrame, HazardDataset]:
        ds_scored_factors = factor_scoring_strategy(
            ds_proj_continuous, ds_hist_continuous, st.session_state.reference_ranges
        )
        ...
        # pass some faulty points to NaN
        fused_score.df.loc[[62, 2904, 2595, 2538], "score"] = np.nan
        # Display only rows where score is not a float
        fused_score = process_score(fused_score, contrast=contrast)
        fused_score = apply_binary_masks(
            fused_score, factors_binary_activation, ds_hist_binary
        )
        fig, ax = plt.subplots()
        fused_score.plot(
            "score", ax=ax, legend=True, cmap="RdYlGn", markersize=MARKER_SIZE
        )
        # Load French departments boundaries from https://raw.githubusercontent.com/gregoiredavid/france-geojson/master/departements.geojson
        depts_path = FPATH.parent.parent.parent.parent / "data" / "departements.geojson"
        departments = gpd.read_file(depts_path).to_crs(METRIC_CRS)
        departments = departments[~departments["nom"].str.lower().str.contains("corse")]
        departments.boundary.plot(ax=ax, color="black", linewidth=0.5, alpha=0.5)
        st.pyplot(fig)
        st.download_button(
            "Download GeoDF", fused_score.to_json(), "heating_map_scores.json"
        )
...
class TopCitiesDisplay:
    ...
    def display(processed_scores):
        ...
        top_rows = get_topn_with_surroundings(
            processed_scores, n=10, score_colname="score"
        ).to_crs(GPS_CRS)
        geometries = top_rows.geometry.to_list()
        coords = [
            tuple(coord[0] for coord in geo.coords.xy[::-1]) for geo in geometries
        ]
...
class CitiesAnalysisDisplayer:
    ...
    def display(processed_scores: gpd.GeoDataFrame, factorwise_scores: HazardDataset):
        ...
        cities = [
            city.strip() for city in cities_str.split(",") if len(city.strip()) >= 3
        ]
        if cities:
            index_of_cities = [
                factorwise_scores.get_index_of_city(city) for city in cities
            ]
            scores_of_cities = processed_scores.iloc[index_of_cities]
            scores_of_cities = scores_of_cities.drop(columns="geometry").assign(
                city=cities
            )
            st.dataframe(scores_of_cities)
            st.dataframe(
                factorwise_scores.df.iloc[index_of_cities]
...
def display():
    if not st.session_state.loaded:
        ...
        (
            ds_hist_binary,
            ds_hist_continuous,
        ) = st.session_state.dataset_ref.split_by_factor_type()

        factors_continous = sorted(
            list(
                set(ds_hist_continuous.factors).intersection(ds_proj_continuous.factors)
            )
        )
        ...
        with map_and_factors_cols[0]:
            (
                factors_continuous_coefs,
                factors_binary_activation,
            ) = WeightsDisplayer.display(factors_continous, factors_binary)
        with map_and_factors_cols[1]:
            processed_scores, factorwise_scores = MapDisplayer.display(
                ds_hist_continuous,
                ds_hist_binary,
                ds_proj_continuous,
                factor_scoring_strategy,
