    ...
    if not st.session_state.loaded:
        ...
        with st.container(border=True):
            with param_cols[0]:
                with st.container(border=True):
                    ...
                    with subparams_cols[0]:
                        factor_scoring_strategy = st.radio("scoring method", FactorwiseScoring.get_available_options(), index=0, key="scoring_method")
            ...
            with param_cols[2]:
                with st.container(border=True):
                    ...
                    with subparams_cols[0]:
                        score_scaling_strategy = st.radio("scale method", ScoreScalingStrategy.get_available_options(), index=0, key="scaling")
                    with subparams_cols[1]:
        ...
        with map_and_factors_cols[0]:
            with st.spinner("Computing score ..."):
                ...
                hazard_map = process_score(dataset_proj, score, contrast=contrast, scaling_strategy=score_scaling_strategy, binary_factor_infos=binary_factor_infos)
