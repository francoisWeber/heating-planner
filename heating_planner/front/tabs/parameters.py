...
def display():
    ...
    with reference_col:
        ...
        with st.container(border=True):
            data_loader = st.selectbox("Dataset type", HazardDatasetType.get_options(), index=0, key="params-select-ref-1")
        ...
        with st.container(border=True):
            data_loader = st.selectbox("Dataset type", HazardDatasetType.get_options(), index=1, key="params-select-ref-2")
            dataset_path = st.text_input("Dataset path", value=sea_elevation_path, key="params-txt-input-ref-2")
            ref_datasets.append(load_and_cache_dataset(dataset_path, data_loader.get_class()))

        with st.container(border=True):
            data_loader = st.selectbox("Dataset type", HazardDatasetType.get_options(), index=2, key="params-select-ref-3")
    ...
    with projection_col:
        st.subheader("Indicate what dataset to use as reference")
        with st.container(border=True):
            data_loader = st.selectbox("Dataset type", HazardDatasetType.get_options(), index=0, key="params-select-proj-1")
        ...
        with st.container(border=True):
            data_loader = st.selectbox("Dataset type", HazardDatasetType.get_options(), index=1, key="params-select-proj-2")
            dataset_path = st.text_input("Dataset path", value=sea_elevation_path, key="params-txt-input-proj-2")
            proj_datasets.append(load_and_cache_dataset(dataset_path, data_loader.get_class()))

        with st.container(border=True):
            data_loader = st.selectbox("Dataset type", HazardDatasetType.get_options(), index=2, key="params-select-proj-3")
