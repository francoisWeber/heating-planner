...
class DriasDataset:
    ...
    def load(self):
        ...
        sections_loc = self.detect_sections_loc(raw_lines)

        self.model = self.get_model_from_lines(raw_lines, sections_loc)
        self.scenario = self.get_scenario_from_lines(raw_lines, sections_loc)
        self.columns_definition = self.get_columns_definition_from_lines(raw_lines, sections_loc)
        self.df = self.get_df_from_lines(raw_lines, sections_loc)

    def __hash__(self):
        return hash(self.path)

    def detect_sections_loc(self, raw_lines: list[str]) -> dict[int, int]:
        return [i for i, line in enumerate(raw_lines) if line.startswith("#---")]

    def get_model_from_lines(self, raw_lines: list[str], sections_loc) -> pd.DataFrame:
    ...
    def get_scenario_from_lines(self, raw_lines: list[str], sections_loc) -> pd.DataFrame:
    ...
    def get_columns_definition_from_lines(self, raw_lines: list[str], sections_loc) -> pd.DataFrame:
    ...
    def get_df_from_lines(self, raw_lines: list[str], sections_loc) -> pd.DataFrame:
    ...
    def get_index_of_city(self, city: str) -> dict:
        ...
        dist_to_loc = self.df.apply(
            lambda row: (row.latitude - loc.latitude) ** 2 + (row.longitude - loc.longitude) ** 2,
            axis=1,
        )
        best_row_index = dist_to_loc.nsmallest(1).index[0]
