...
class SeaElevationDataset(HazardDataset):
    ...
    @staticmethod
    def process_raw_sea_elevation_dataset(
        sea_elevation_path: str,
        reference: gpd.GeoDataFrame | HazardDataset,
        output_path: str,
    ):
        ...
        sea_df = gdf[["geometry", "FID_histol"]].rename(
            columns={"FID_histol": FACTOR_NAME}
        )
        ...
        scaled_sea_df = gpd.sjoin_nearest(
            reference[["geometry"]], sea_df, how="left", max_distance=MAX_DISTANCE_M
        ).drop(columns="index_right")

        logger.info("Casting sea elevation to binary values 'location submerged'")
        scaled_sea_df[FACTOR_NAME] = (1 - scaled_sea_df[FACTOR_NAME].fillna(1)).astype(
            bool
        )
