...
class RealEstatePricesEuroPerM2(HazardDataset):
    ...
    def _filter_dvf_df(df: pd.DataFrame) -> pd.DataFrame:
        df = (
            df[df.type_local.isin(["Maison", "Appartement"])]
    ...
    @staticmethod
    def _merge_dvf_data_onto_ref_geometry(
        dvf_df: gpd.GeoDataFrame, ref_df: gpd.GeoDataFrame
    ) -> gpd.GeoDataFrame:
        gdf = gpd.sjoin_nearest(
            ref_df.to_crs(METRIC_CRS),
            dvf_df.to_crs(METRIC_CRS),
            how="left",
            max_distance=SJOIN_MAX_DIST_AGG_DVF,
        )
        return gpd.GeoDataFrame(
            gdf.groupby("geometry")[FACTOR_NAME].mean().reset_index(),
            geometry="geometry",
        ).to_crs(METRIC_CRS)
    ...
    def _convert_dvf_to_gdf(df: pd.DataFrame) -> gpd.GeoDataFrame:
        return gpd.GeoDataFrame(
            df, geometry=gpd.points_from_xy(df.longitude, df.latitude), crs=GPS_CRS
    ...
    @staticmethod
    def prepare_dvf_data_wrt_reference_data(
        dvf_paths: str | List[str], ref_ds: HazardDataset, output_dir: str
    ) -> None:
        ...
        gdf = RealEstatePricesEuroPerM2._merge_dvf_data_onto_ref_geometry(
            gdf, ref_ds.df
        )
